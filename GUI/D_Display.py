...
from GB_music.GUI.M_Display import GB_music
from GB_music.Cloud_Music import Cloud_Music_API
import threading
import random
import requests
import time
...
# 主窗口
class Main_window(QWidget, GB_music):
    def __init__(self):
    ...
    def set_first_date(self):
        if self.src_choose.currentText() == "网易云":
            ...
            self.listview.clear()
            self.date.img_url, self.date.bt_name, self.date.bt_id = Cloud_Music_API.wyy_first_page()
            for i in range(8):
                img = QPixmap()
    ...
    def first_playlist_dj(self):
        ...
        if self.src_choose.currentText() == "网易云":
            if id_str[1] == 'p':
                self.date.music_name, self.date.music_id = Cloud_Music_API.playlist_info(id_str.split('=')[1])
                ...
                self.player.setMedia(QMediaContent(QUrl(Cloud_Music_API.dj_url(id_str.split('=')[1]))))  # 设置音频网址
    ...
    def list_view_song(self):
        ...
        if self.listview_lable.objectName() == "local":
            ...
            if src == "网易云":
                url = Cloud_Music_API.song_url(num)
                self.player_stop_setting()
            ...
            if self.src_choose.currentText() == "网易云":
                url = Cloud_Music_API.song_url(music_id)
                if url == "":
    ...
    def show_search(self):
        self.lock.acquire()
        if self.src_choose.currentText() == "网易云":
            self.date.music_name, self.date.music_id = Cloud_Music_API.single_search(self.search_edit.text())
            self.listview.clear()
            self.listview_lable.setObjectName("internet")
    ...
    def closeEvent(self, event):
        result = QMessageBox.question(self, "退出", "确认退出播放器吗?", QMessageBox.Yes | QMessageBox.No)
